...
exports.getUsersFromExactNamespace = async (req,res,next) => {

  let err = new Error();
  if(!io){
    err = {
      msg:'Error while getting socketio io instance',
      status:500
    }
    return next(err);
  }
  // const ids = await io.of('/'+req.params.namespace).allSockets();
  ...
  if(!io){
    err = {
      msg:'Error while getting whole users via socketio',
      status:500
    }
    return next(err);
  }
  res.status(200).json({
    success:true,
    msg:'Success. You get all users from '+req.params.namespace+' namespace.',
    data:ids,
    status:200
  })
}
...
exports.postFileUpload = (req, res) => {
  ...
  console.log(req);
  return res.status(200).json({ success:true, msg: 'File was uploaded successfully.' });
};
